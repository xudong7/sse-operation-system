...
#include <time.h>
#include <sys/time.h>
#include <sched.h>  // 添加用于 sched_yield()

#define BUFFER_SIZE 1000  // 增大缓冲区
#define NUM_PRODUCERS 3
#define NUM_CONSUMERS 3
#define NUM_ITEMS 1000000 // 处理的数据量
#define SPIN_YIELD_THRESHOLD 100 // 自旋超过此次数则让出CPU
...
void *producer(void *arg)
{
    ...
    for (int i = 0; i < NUM_ITEMS; i++)
    {
        ...
        while (1) {
            ...
            // 避免CPU过度自旋，适当让出CPU
            spin_count++;
            if (spin_count % SPIN_YIELD_THRESHOLD == 0) {
                sched_yield();
            }
        }
